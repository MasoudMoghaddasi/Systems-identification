...
for i = 1 : floor( length( u ) / Q )
    ...
    theta( : , i ) = ( ( X' * X )^-1 * X' * y_Train ) ./ X_normalize_term ;
    
    e_Train = X * theta( : , i ) - y_Train ;
    
    Var_Noise_estim( 1 , i ) = ( e_Train' * e_Train ) / ( 0.75 * Q - n ) ;
    
    COV_Theta( : , : , i ) = Var_Noise_estim( 1 , i ) * ( X' * X )^-1 ;
end
...
% cov_Theta = varN_es * ( X_Test' * X_Test )^-1 ;

% cov_Theta = mean( COV_Theta , 3 ) ;
...
fprintf('    Parameter     Estimated Value  Real Value\n'  )
...
ylabel( 'y' )
title( 'Errorbars' )
legend( ' y_h_a_t ' , ' y ' , ' max error ' , ' min error ' )
